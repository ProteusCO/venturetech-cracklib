...
public class Packer
{
     ...
     * @param name the base name of the packer files, ".pwd", etc. will be appended.
     * @param mode "r" or "rw".
     * @throws IOException if an I/O error occurs.
     */
    public Packer(String name, String mode) throws IOException
    {
        this(name+".pwd", name+".pwi", name+".hwm", mode);
    }
    
    /**
     * Create a new instance.
     * @param pwd the pwd file name.
     * @param pwi the pwi file name.
     * @param hwm the hwm file name.
     * @param mode "r" or "rw".
     * @throws IOException if an I/O error occurs.
     */
    public Packer(String pwd, String pwi, String hwm, String mode) throws IOException
    {
        this.mode = mode;
        ...
        dataFile = new RandomAccessFile(pwd, mode); // data file
        indexFile = new RandomAccessFile(pwi, mode); // index file
        try
        {
            hashFile = new RandomAccessFile(hwm, mode); // hash file
        }
        catch (IOException e)
        {
    ...
    public synchronized void put(String s) throws IOException
    {
        ...
        data[count] = s.length() > MAXWORDLEN - 1 ? s.substring(0,
                MAXWORDLEN - 1) : s;
